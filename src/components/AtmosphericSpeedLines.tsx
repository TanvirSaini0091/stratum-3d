...
const Starfield: React.FC<StarfieldProps> = ({
  ...
  const bigBang = () => {
    ...
    if (sd.current.star.arr.length !== qty) {
      sd.current.star.arr = new Array(qty).fill(0).map(() => [
  ...
  const update = () => {
    ...
    const compSpeed = currentProps.hyperspace ? currentProps.speed * currentProps.warpFactor : currentProps.speed;
...
// ==========================================
export function AtmosphericSpeedLines({ descentState }: { descentState: DescentState }) {
